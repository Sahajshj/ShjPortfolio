...
async function startServer() {
  ...
  // Serve the resume PDF directly as static downloads
  app.get("/Sahajpal_Cloud_Engineer.pdf", (req, res) => {
    res.download(path.join(process.cwd(), "Sahajpal_Cloud_Engineer.pdf"), "Sahajpal_Cloud_Engineer.pdf");
  });
  app.get("/Sahajpal_Singh_Cloud_Engineer.pdf", (req, res) => {
    res.download(path.join(process.cwd(), "Sahajpal_Singh_Cloud_Engineer.pdf"), "Sahajpal_Singh_Cloud_Engineer.pdf");
  });
  ...
  app.post("/api/send-email", async (req, res) => {
    try {
      ...
      const targetEmail = process.env.NEXT_PUBLIC_NOTIFICATION_EMAIL || "sahajpal1905@gmail.com";
      ...
      if (!response.ok) {
        ...
        return res.status(response.status).json({ error: data.message || "Failed to deliver message via Resend." });
      }
